...
int test_differ()
{
    ...
    {
        // 1d differentiation
        auto diff_1d = differ::diff(data_1d);
        utils::WriteData(
            "test_result/differ/diff_1d.txt", diff_1d, 1, diff_1d.size());

        // 2d differentiation along rows
        auto diff_2d_row = differ::diff(ori_matrix, 0);
        auto diff_2d_row_vec = std::vector<double>(
            diff_2d_row.data(), diff_2d_row.data() + diff_2d_row.size());
        utils::WriteData("test_result/differ/diff_2d_row.txt",
                         diff_2d_row_vec,
                         diff_2d_row.cols(),
                         diff_2d_row.rows());

        // 2d differentiation along columns
        auto diff_2d_col = differ::diff(ori_matrix, 1);
        auto diff_2d_col_vec = std::vector<double>(
            diff_2d_col.data(), diff_2d_col.data() + diff_2d_col.size());
        utils::WriteData("test_result/differ/diff_2d_col.txt",
                         diff_2d_col_vec,
                         diff_2d_col.cols(),
                         diff_2d_col.rows());

        // 1d central gradient
        auto cent_grad_1d = differ::central_gradient(data_1d, 1.0);
        utils::WriteData("test_result/differ/cent_grad_1d.txt",
                         cent_grad_1d,
                         1,
                         cent_grad_1d.size());

        // 2d central gradient along rows
        auto cent_grad_2d_row = differ::central_gradient(ori_matrix, 1.0, 0);
        auto cent_grad_2d_row_vec = std::vector<double>(
            cent_grad_2d_row.data(),
            cent_grad_2d_row.data() + cent_grad_2d_row.size());
        utils::WriteData("test_result/differ/cent_grad_2d_row.txt",
                         cent_grad_2d_row_vec,
                         cent_grad_2d_row.cols(),
                         cent_grad_2d_row.rows());

        // 2d central gradient
        auto cent_grad_2d_col = differ::central_gradient2d(ori_matrix, 1.0);
        ...
        utils::WriteData("test_result/differ/cent_grad_2d_col_x.txt",
        ...
        utils::WriteData("test_result/differ/cent_grad_2d_col_y.txt",
